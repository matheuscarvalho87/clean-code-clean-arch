...
test("Deve testar o ORM", async ()=>{
  ...
  expect(savedAccountModel.isPassenger).toBe(true)
  await connection.close()
})
test("Deve testar o ORM com um aggregate real", async ()=>{
  const accountId = crypto.randomUUID();
  const account = Account.restore(accountId, "John Doe", "john.doe@gmail.com", "97456321558", true, false, "")
  const accountModel = AccountModel.fromAggreagate(account)
  const connection = new PgPromiseAdapter();
  const orm = new ORM(connection);
